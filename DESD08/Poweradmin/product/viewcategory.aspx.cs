...
public partial class Poweradmin_ProductCategory_viewProductCategory : System.Web.UI.Page
{
    int findex, lindex;
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "productcategory", MessageNm = "Product Category";
    ...
    protected bool Delete_Data(string id)
    {
        try
        {
            string albumimage = new DAL().ExecuteScalar("select count(*) from product where catid=" + id, CommandType.Text, null);
            string deal = new DAL().ExecuteScalar("select count(*) from deal where catid=" + id, CommandType.Text, null);
            if (albumimage != "0" )
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('Product Category Cannot be deleted as there are " + albumimage + " Product(s) in this category');", true);
                return false;
            }
            if ( deal != "0")
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('Product Category Cannot be deleted as there are " + deal + " Deal(s) in this category');", true);
                return false;
            }

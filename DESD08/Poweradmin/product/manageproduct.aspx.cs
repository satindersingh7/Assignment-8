...
public partial class Poweradmin_Product_manageProduct : System.Web.UI.Page
{
    ...
    protected void btn_Submit_Click(object sender, EventArgs e)
    {
        ...
        if (fulp.HasFile)
        {
            ...
            else
            {
                ...
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('Product image width and height should be greater than " + ConfigurationManager.AppSettings["ProductImgHeight"].ToString() + "px and " + ConfigurationManager.AppSettings["ProductImgWidth"].ToString() + "px');", true);
                Detail = "active"; Image = ""; Associate = "";
                return; 
            }
        ...
        if (!DAL.IsNumeric(Request.QueryString["id"]))
        {
            string code = (new DAL()).ExecuteScalar("Select count(*) from Product where productcode='" + txtProductCode.Text + "'", CommandType.Text, null); // this query will fetch the maximum display order number from db
            if (code != "0")
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('Product Code already used.');", true);
                Detail = "active"; Image = ""; Associate = "";
                return;
            }
            ...
            #endregion
            strQuery = "insert into Product(Visible,outofstock,featured,Title,productcode,catid,image,costprice,sellprice,description,Perweek,sortorder,AddedDate,AddedIp) ";
            strQuery += "values('" + rbl_Visible.SelectedValue.ToString() + "','" + rblstockout.SelectedValue.ToString() + "',";
            ...
            msg = "Product details has been added successully.";
        ...
        {
            string code = (new DAL()).ExecuteScalar("Select count(*) from Product where id<>" + Request.QueryString["id"].ToString()+" and productcode='" + txtProductCode.Text + "'", CommandType.Text, null); // this query will fetch the maximum display order number from db
            if (code != "0")
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('Product Code already used.');", true);
                Detail = "active"; Image = ""; Associate = "";
            ...
            strQuery = "update Product set Visible='" + rbl_Visible.SelectedValue.ToString() + "',outofstock='" + rblstockout.SelectedValue.ToString() + "',";
           ...
           {
               string strQuery1 = "update Product set sortorder=sortorder-1 where Catid=" + ViewState["catid"].ToString() + " and sortorder>" + ViewState["sortorder"].ToString();
               new DAL().ExecuteNonQuery(strQuery1, CommandType.Text, null);

               string maxnum = (new DAL()).ExecuteScalar("Select max(sortorder) from Product where Catid=" + ddl_Category.SelectedValue.ToString() + " and id<>" + Request.QueryString["id"].ToString(), CommandType.Text, null); // this query will fetch the maximum display order number from db
               if (maxnum != null && maxnum != "")
                   sortorder = int.Parse(maxnum.ToString()) + 1;

               strQuery1 = "update Product set sortorder=" + sortorder + " where id=" + Request.QueryString["id"].ToString();
               new DAL().ExecuteNonQuery(strQuery1, CommandType.Text, null);
           }
            ...
            (new DAL()).ExecuteNonQuery(strQuery, CommandType.Text, null);
            msg = "Product details has been modified successfully.";
            Response.Redirect("viewProduct.aspx?msg=" + msg);
    ...
    {
        string strQuery = "select * from Product where id=" + Request.QueryString["id"].ToString();
        DataTable dt = new DAL().GetDataTable(strQuery, CommandType.Text, null);
        if (dt != null && dt.Rows.Count > 0)
        {
            rbl_Visible.SelectedValue = dt.Rows[0]["Visible"].ToString().Replace("True", "1").Replace("False", "0");
            rblstockout.SelectedValue = dt.Rows[0]["outofstock"].ToString().Replace("True", "1").Replace("False", "0");
    ...
    protected void btn_Submit_Images_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            Response.Redirect("viewproduct.aspx?msg=Product has been saved Successfully");
        }
        catch (Exception ex)
        {
            throw ex;
    ...
    protected void Fill_Images()
    {
        try
        {
            strQuery = "select * from product_images where productid = " + Request.QueryString["id"].ToString() + " order by id ";
    ...
    protected void SaveImages(Dictionary<string, string> Names, object PID)
    {
        ...
        foreach (KeyValuePair<string, string> item in Names)
        {
            ...
            strQuery = "insert into product_images(ProductId,image) values(" + int.Parse(PID.ToString()) + ",'" + item.Key.Replace("'", "''") + "')";
             (new DAL()).ExecuteNonQuery(strQuery, CommandType.Text, null);
        }
    ...
    protected void RepExistingImages_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        ...
        {
            string SELQuery = "SELECT image FROM product_images WHERE ID = " + e.CommandArgument.ToString();
            ...
            (new DAL()).ExecuteNonQuery("delete from product_images where id=" + e.CommandArgument.ToString(), CommandType.Text, null);
    ...
    protected void fill_associated()
    {
        ...
        string strQuery1 = "select p.* from AssociatedProduct ap,Product p where p.id=ap.AssociatePID and ap.pid=" + Request.QueryString["id"].ToString() + " order by p.Title";
    ...
    {
        ListItem li = new ListItem("Product does not exists", "");
        ...
        {
            ListItem l1 = new ListItem("No Products found", "");
            lbproduct.Items.Add(l1);
        }
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        ...
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('Select maximum 4 associate product.');", true);
            Detail = ""; Image = ""; Associate = "active";
            return;
        }
        else
        {
            string strQuery = "select * from AssociatedProduct where Pid=" + Request.QueryString["id"].ToString();
            DataTable dt = new DAL().GetDataTable(strQuery, CommandType.Text, null);
            if (dt != null && dt.Rows.Count > 0)
            {

                strQuery = "delete from AssociatedProduct where pid=" + Request.QueryString["id"].ToString();
                (new DAL()).ExecuteNonQuery(strQuery, CommandType.Text, null);
            }
            foreach (ListItem li in lbassociate.Items)
            {
                strQuery = "insert into AssociatedProduct(Pid,AssociatePID) values('" + Request.QueryString["id"].ToString() + "','" + li.Value.ToString() + "')";
                (new DAL()).ExecuteNonQuery(strQuery, CommandType.Text, null);

            }
            Response.Redirect("viewproduct.aspx?msg=Product succcessfully associated with another products");
        }
    }

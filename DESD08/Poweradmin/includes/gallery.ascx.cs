...
public partial class Poweradmin_includes_subcat : System.Web.UI.UserControl
{
    int findex, lindex;
    ...
    protected bool Delete_Data(string id)
    {
        try
        {
            ...
            string cat1 = (new DAL()).ExecuteScalar("select catid from " + TableName + " where id=" + id, CommandType.Text, null);
              strQuery = "update " + TableName + " set sortorder = sortorder - 1 where catid=" + cat1 + " and sortorder >  (select sortorder from (select sortorder from " + TableName + " where id =" + id + ") as sortorder)";
            new DAL().ExecuteNonQuery(strQuery, CommandType.Text, null);
    ...
    protected string GetImage(string image)
    {
        ...
        {
            path = "<img src='../../webfiles/photogallery/medium/" + image + "' class='img-responsive' style='width:100px' />";
        }
        return path;
    }

...
public partial class Poweradmin_teaser_viewteaser : System.Web.UI.Page
{
    int findex, lindex;
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "teaser", MessageNm = "Advertisement Banner";
    ...
    {
        string strQuery = "select * from teaser where id<>0";
        if (ddlPostion.SelectedValue.ToString() != "") strQuery += " and position=" + ddlPostion.SelectedValue.ToString();
    ...
    protected void ddlDisplayOrder_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        HiddenField hdnProId1 = (HiddenField)(rep.Items[rowIndex1].FindControl("hfLayoutId"));
        ...
        try
        {
            if (oldOrder < selectedOrder)
                strsort = "update " + TableName + " set sortorder = sortorder - 1 where sortorder > " + oldOrder + " and sortorder <= " + selectedOrder + " and position=" + ddlPostion.SelectedValue.ToString();
            else
                strsort = "update " + TableName + " set sortorder = sortorder + 1 where sortorder >= " + selectedOrder + " and sortorder < " + oldOrder + " and position=" + ddlPostion.SelectedValue.ToString();

            new DAL().ExecuteNonQuery(strsort, CommandType.Text, null);
            strsort = "update " + TableName + " set sortorder=" + selectedOrder + " where id=" + id1 + " and position=" + ddlPostion.SelectedValue.ToString();
            new DAL().ExecuteNonQuery(strsort, CommandType.Text, null);
        }
        catch (Exception ex)
        {
    ...
    protected void Hide_Show(int status)
    {
        ...
        foreach (RepeaterItem Ri in rep.Items)
        {
            ...
            HiddenField hfNewsId = (HiddenField)Ri.FindControl("hfLayoutId");//this will find the hidden control 'hfNewsId'
    ...
    protected bool Delete_Data(string id)
    {
        try
        {
            ...
            try
            {
                if (File.Exists(DAL.GetRootPath() + "\\webfiles\\teaser\\" + FileName))
                    File.Delete(DAL.GetRootPath() + "\\webfiles\\teaser\\" + FileName);
    ...
    {
        ddlPostion.SelectedValue = ddlvisble.SelectedValue =  "";
        CurrentPage = 0;
        fill_Data();
    }
    ...
    protected string GetImage(string image)
    {
        ...
        if (image == "")
        {
            path = "<img src='../../webfiles/img_not_available.png' style='height: 100px !Important; width:100px' class='img-responsive' />";
        }
        else
        {
            path = "<img src='../../webfiles/teaser/" + image + "' class='img-responsive' style='height: 100px !Important; width:100px' />";
        }
        return path;
    }

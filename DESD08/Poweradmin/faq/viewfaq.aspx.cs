...
public partial class Poweradmin_faq_viewfaq : System.Web.UI.Page
{
    int findex, lindex;
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "faq", MessageNm = "Faq";
    ...
    protected void fill_Data()
    {
        ...
        if (tbx_Search.Text != "") strQuery += " and Question like '%" + tbx_Search.Text.Replace("'", "''") + "%'";
    ...
    protected bool Delete_Data(string id)
    {
        try
        {
            ...
            if (FileName != "")
            {
                try
                {
                    if (System.IO.File.Exists(DAL.GetRootPath() + "\\webfiles\\tips\\" + FileName))
                        System.IO.File.Delete((DAL.GetRootPath() + "\\webfiles\\tips\\" + FileName));
                }
                catch (Exception ex)
                { }
            }
    ...
    {
      ddlvisble.SelectedValue = tbx_Search.Text = "";
        CurrentPage = 0;
        fill_Data();
    }

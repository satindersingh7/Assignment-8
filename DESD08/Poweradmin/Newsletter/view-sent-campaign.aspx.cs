...
public partial class Poweradmin_Newsletter_view_sent_campaign : System.Web.UI.Page
{
    int findex, lindex;
    ...
    {
        string strQuery = "select *,(select count(id) from newslettersendhistory where campaignid=" + TableName + ".id) as pending, subscribercnt-(select count(id) from newslettersendhistory where campaignid=" + TableName + ".id) as sent from " + TableName + " where status=" + (int)CampaignStatus.Sent + " ";

...
public partial class Poweradmin_Newsletter_viewgroup : System.Web.UI.Page
{
    int findex, lindex;
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "newslettergroup", MessageNm = "Group";
    ...
    {
        string strQuery = "select * from " + TableName + " where id<>1";
    ...
    protected bool Delete_Data(string id)
    {
        try
        {
            string tot = new DAL().ExecuteScalar("Select count(*) from newslettersubscriber where groupid=" + id, CommandType.Text, null);
            if (tot != "0")
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.error('System can not delete this group because " + tot + " subscriber exists under it.');", true);                
                return false;
            }

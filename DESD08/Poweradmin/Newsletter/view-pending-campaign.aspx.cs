...
public partial class Poweradmin_Newsletter_view_pending_campaign : System.Web.UI.Page
{
    int findex, lindex;
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "newslettercampaign", MessageNm = "Newsletter Campaign";
    ...
    {
        string strQuery = "select *,(select count(id) from newslettersendhistory where campaignid=" + TableName + ".id) as pending, subscribercnt-(select count(id) from newslettersendhistory where campaignid=" + TableName + ".id) as sent from " + TableName + " where status=" + (int)CampaignStatus.Pending + " ";
        //if (tbx_Search.Text != "") strQuery += " and subject like '%" + tbx_Search.Text.Replace("'", "''") + "%'";
    ...
    protected void Hide_Show(int status)
    {
        ...
        if (status == 0)
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.success('Selected " + MessageNm + " record has been successfully set to active.');", true);
        else
            ScriptManager.RegisterStartupScript(this, this.GetType(), "ShowPopup", "toastr.success('Selected " + MessageNm + " record has been successfully set to block.');", true);

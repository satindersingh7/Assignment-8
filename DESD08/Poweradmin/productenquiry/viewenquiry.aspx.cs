...
public partial class Poweradmin_PEnquiry_viewenquiry : System.Web.UI.Page
{
    int findex, lindex;
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "ordermaster", MessageNm = "Product Enquiry";
    ...
    protected void fill_Data()
    {
        ...
        if (tbx_Search.Text != "") strQuery += " and FName like '%" + tbx_Search.Text.Replace("'", "''") + "%'";
        if (ddlstatus.SelectedValue != "") strQuery += " and orderstatus = '" + ddlstatus.SelectedValue + "'";
        if (tbx_SearchLastName.Text != "") strQuery += " and  email like '%" + tbx_SearchLastName.Text.Replace("'", "''") + "%'";
        if (tbx_Date.Text != "") strQuery += " and date(orderDate)='" + DateTime.Parse(tbx_Date.Text.Replace("'", "''")).ToString("yyyy-MM-dd") + "'";

...
public partial class Poweradmin_order_viewWholesale : System.Web.UI.Page
{
    public static int pageindex = 0, Pagesize = 25;
    string strQuery, TableName = "wsordermaster", MessageNm = "Wholesale Order";
    ...
    protected void fill_Data()
    {
        ...
        strQuery = "select t.* from " + TableName + " t where t.id<>0";
        if (tbx_Search.Text != "") strQuery += " and t.orderid like '%" + tbx_Search.Text.Replace("'", "''") + "%' or t.cname like '%" + tbx_Search.Text.Replace("'", "''") + "%'";
        if (ddl_Status.SelectedValue.ToString() != "") strQuery += " and t.status = " + ddl_Status.SelectedValue.ToString() + "";
        if (tbx_FromDate.Text != "" && tbx_ToDate.Text == "")
            strQuery += " and date(t.addeddate)='" + DateTime.Parse(tbx_FromDate.Text.Replace("'", "''")).ToString("yyyy-MM-dd") + "' ";
        else if (tbx_ToDate.Text != "" && tbx_FromDate.Text == "")
            strQuery += " and date(t.addeddate)='" + DateTime.Parse(tbx_ToDate.Text.Replace("'", "''")).ToString("yyyy-MM-dd") + "' ";
        else if (tbx_ToDate.Text != "" && tbx_FromDate.Text != "" && tbx_FromDate.Text == tbx_ToDate.Text)
            strQuery += " and date(t.addeddate)='" + DateTime.Parse(tbx_ToDate.Text.Replace("'", "''")).ToString("yyyy-MM-dd") + "' ";
        else if (tbx_ToDate.Text != "" && tbx_FromDate.Text != "")
            strQuery += " and date(t.addeddate)>='" + DateTime.Parse(tbx_FromDate.Text.Replace("'", "''")).ToString("yyyy-MM-dd") + "' and date(t.addeddate)<='" + DateTime.Parse(tbx_ToDate.Text.Replace("'", "''")).ToString("yyyy-MM-dd") + "' ";

        strQuery += " order by t.id desc OFFSET 0 Rows Fetch Next " + Pagesize + " Rows Only  ";

        strQuery += strQuery.Replace("t.*", "count(t.id) as count");
    ...
    protected bool Delete_Data(string id)
    {
        try
        {
            ...
            strQuery = "DELETE FROM wsorderitem WHERE orderid=" + id;
            new DAL().ExecuteNonQuery(strQuery, CommandType.Text, null);
